...
(function($) {
	...
	require.prototype = {
		_init: function(b) {
			...
			$('body').on({
				...
				focus: function(a) {
					b.onFocus ? b.onFocus($(this)) : $(this).parent().find("span.focus").not(".valid").removeClass("hide").siblings(".valid").addClass("hide") && $(this).parent().find(".blank").addClass("hide") && $(this).parent().find(".close").addClass("hide")
				},
				keyup: function(a) {
					if (b.phone && $(this).attr("id") === "phone") d._change($(this))
				},
	...
	j._resultTips = function(a, b, c) {
		a.parent().find("span.focus").not(".valid").addClass("hide").siblings(".focus").removeClass("hide");
		a.parent().find(".close").addClass("hide");
		a.removeClass("v_error");
		c = c || "";
		if (c.length > 21) c = "<span>" + c + "</span>";
		var o = a.parent().find("span.valid");
		if (!b) {
			o.addClass("error");
			a.addClass("v_error");
